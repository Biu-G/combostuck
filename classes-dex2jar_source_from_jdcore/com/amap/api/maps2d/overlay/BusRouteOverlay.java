  ...
  extends b
{
  ...
  {
    return mAMap.addPolyline(new PolylineOptions().add(new LatLng[] { paramLatLng1, paramLatLng2 }).width(getBuslineWidth()).color(getWalkColor()));
  }
  ...
  {
    return mAMap.addPolyline(new PolylineOptions().addAll(paramList).color(getWalkColor()).width(getBuslineWidth()));
  }
  
  private void a(LatLng paramLatLng, String paramString1, String paramString2)
  {
    paramLatLng = mAMap.addMarker(new MarkerOptions().position(paramLatLng).title(paramString1).snippet(paramString2).visible(mNodeIconVisible).anchor(0.5F, 0.5F).icon(getWalkBitmapDescriptor()));
    stationMarkers.add(paramLatLng);
  }
  ...
  private void a(BusStep paramBusStep)
  {
    ...
    while (i < paramBusStep.size())
    {
      ...
      if (localObject1 != null)
      {
        ...
        b = ((LatLng)((List)localObject1).get(((List)localObject1).size() - 1));
  ...
  private void a(RouteBusLineItem paramRouteBusLineItem)
  {
    ...
    paramRouteBusLineItem = mAMap.addPolyline(new PolylineOptions().addAll(paramRouteBusLineItem).color(getBusColor()).width(getBuslineWidth()));
    allPolyLines.add(paramRouteBusLineItem);
  }
  
  private String b(List<WalkStep> paramList)
  {
    paramList = paramList.iterator();
    for (float f = 0.0F; paramList.hasNext(); f += ((WalkStep)paramList.next()).getDistance()) {}
  ...
  private void b(RouteBusLineItem paramRouteBusLineItem)
  {
    ...
    paramRouteBusLineItem = mAMap.addMarker(new MarkerOptions().position(a.a(localBusStationItem.getLatLonPoint())).title(paramRouteBusLineItem.getBusLineName()).snippet(c(paramRouteBusLineItem)).anchor(0.5F, 0.5F).visible(mNodeIconVisible).icon(getBusBitmapDescriptor()));
    stationMarkers.add(paramRouteBusLineItem);
  }
  ...
  private String c(RouteBusLineItem paramRouteBusLineItem)
  {
    ...
    localStringBuilder.append(paramRouteBusLineItem.getDepartureBusStation().getBusStationName());
    localStringBuilder.append("-->");
    localStringBuilder.append(paramRouteBusLineItem.getArrivalBusStation().getBusStationName());
    localStringBuilder.append(") 经过");
    localStringBuilder.append(paramRouteBusLineItem.getPassStationNum() + 1);
    localStringBuilder.append("站");
  ...
  private void c(BusStep paramBusStep1, BusStep paramBusStep2)
  {
    ...
    if (!paramBusStep1.equals(paramBusStep2))
    {
      ...
      allPolyLines.add(paramBusStep1);
    }
  }
    ...
    for (;;)
    {
      ...
      try
      {
        ...
        if (i < localList.size())
        {
          ...
          else if (((BusStep)localObject1).getBusLine() == null)
          {
            localObject2 = a(b, endPoint);
            allPolyLines.add(localObject2);
          }
